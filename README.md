...
- **TOML Configuration**: Persistent config at `~/.config/fsmon/fsmon.toml` (or legacy `config.toml` for backward compatibility)
...
1. `~/.fsmon/fsmon.toml` — home directory (new naming)
2. `~/.config/fsmon/fsmon.toml` — XDG standard path (generated by `fsmon generate`)
3. `/etc/fsmon/fsmon.toml` — system-wide (new naming)
4. `~/.fsmon/config.toml` — legacy backward-compatible
