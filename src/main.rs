...
支持的 14 种事件类型（与 fanotify 一一对应）：
  ACCESS, MODIFY, CLOSE_WRITE, CLOSE_NOWRITE, OPEN, OPEN_EXEC,
  ATTRIB, CREATE, DELETE, DELETE_SELF, MOVED_FROM, MOVED_TO,
  MOVE_SELF, FS_ERROR
...
【过滤选项】
  ...
  --types     仅监控指定事件类型 (CREATE,MODIFY,CLOSE_WRITE,...)
  --exclude   排除匹配通配符的路径 (如："*.log", "/tmp/*")
  --all-events 开启全部 14 种 fanotify 事件（默认只有 8 种变更事件）
...
【事件过滤】
  --types     按事件类型过滤 (ACCESS,MODIFY,CREATE,DELETE,MOVED_FROM,MOVED_TO,...)
