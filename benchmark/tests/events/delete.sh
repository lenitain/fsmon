...
# 先创建文件（不记录事件）
n=50
for i in $(seq 1 $n); do echo "to_delete" > "$BENCH_DIR/file_$i.txt"; done

# 启动 daemon + 注册监控
restart_daemon
register

info "=== DELETE: 删除 50 个文件 ==="
for i in $(seq 1 $n); do rm "$BENCH_DIR/file_$i.txt"; done
sleep 5
...
count=$(count_type "DELETE")
info "期望 = $n，实际: $count"
[ "$count" -eq "$n" ] && ok "DELETE 数量" || fail "DELETE 数量 (期望 $n, 实际 $count)"

cleanup

echo ""
echo -e "  DELETE 测试: ${GREEN}${passed} passed${NC}  ${RED}${failed} failed${NC}"
[ "$failed" -eq 0 ]
