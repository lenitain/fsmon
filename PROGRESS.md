...
fsmon 共有 **6 个缓存系统** + **1 个守护进程锁**，分布在 5 个模块中，覆盖路径解析、进程信息、文件大小、用户映射和进程树追踪。

| # | 缓存 | 位置 | 类型 | 容量 | 淘汰策略 |
|---|------|------|------|------|----------|
| 1 | 目录 Handle 缓存 | `dir_cache.rs` / `fid_parser.rs` | `moka::sync::Cache<HandleKey, PathBuf>` | 100,000 | W-TinyLFU + TTL 1h |
| 2 | 进程信息缓存 | `proc_cache.rs` + `monitor.rs` | `Arc<DashMap<u32, ProcInfo>>` | 无上限 | 无淘汰，永不失效 |
| 3 | 进程树缓存 | `proc_cache.rs` | `Arc<DashMap<u32, PidNode>>` | 无上限 | 无淘汰，永不失效 |
| 4 | 文件大小缓存 | `monitor.rs` | `LruCache<PathBuf, u64>` | 10,000 | LRU |
| 5 | PID 信息缓存（第二层） | `monitor.rs` | `LruCache<u32, ProcInfo>` | 4,096 | LRU |
...
#### 2. 进程信息缓存（`DashMap<u32, ProcInfo>`）

**位置**：`proc_cache.rs`，定义 `ProcCache = Arc<DashMap<u32, ProcInfo>>`
...
⚠️ **问题**：
- **无上限增长**：Linux PID 上限通常约 4 万~400 万，长期运行可能积累大量已退出进程的条目
- **Exit 事件不清理**：`handle_proc_events` 中明确注释不清理。这在查询历史时合理，但实时事件处理中会积累大量僵尸条目。PID 重用场景下可能导致错误的进程信息关联
- **两套 PID 缓存并存不一致**：`proc_cache`（DashMap，事件驱动写入）和 `pid_cache`（LruCache，惰性填充）存储相同类型的数据，但信息源可能不同步。proc_cache 由 proc connector 实时更新，但 pid_cache 写入后不再刷新。如果某个进程紧接着执行了新 exec（命令），proc_cache 已更新，但 pid_cache 中还是旧数据，直到被 LRU 淘汰

---

#### 3. 进程树缓存（`DashMap<u32, PidNode>`）

**位置**：`proc_cache.rs`，定义 `PidTree = Arc<DashMap<u32, PidNode>>`
...
#### 5. PID 信息缓存（第二层，`LruCache<u32, ProcInfo>`）

**位置**：`monitor.rs`，`Monitor.pid_cache`

**容量**：4,096

**用途**：与 proc_cache 存储相同类型的数据，但用 LRU 淘汰机制控制内存。高频 PID 留在 LruCache，低频的被淘汰，仍可从 proc_cache 或 `/proc` 重建。

**工作流程**：
```
build_file_event_for_opts:
  1. 查 pid_cache (LruCache) → 命中则返回
  2. 未命中 → 调用 get_process_info_by_pid()
     - 先查 proc_cache (DashMap) 
     - 再回退读 /proc
  3. 将结果写入 pid_cache (LruCache)
```

**评价**：

✅ **优点**：
- **LRU 淘汰**：内存可控，不会被僵尸 PID 撑爆
- **冷热分离**：高频 PID 常驻 LruCache，低频的可重建

⚠️ **问题**：
- **两套 PID 缓存不一致**（同上，见第 2 条）
- **容量 4096 vs 10000 差异缺乏说明**：为什么 `file_size_cache` 是 10000 而 `pid_cache` 是 4096？缺乏注释解释
...
| 问题 | 严重程度 | 说明 |
|------|----------|------|
| `proc_cache` 无淘汰机制 | **中** | DashMap 无限增长，长期运行可能 OOM |
| 两套 PID 缓存不一致 | **中** | proc_cache vs pid_cache 可能版本不同步 |
| Exit 事件不清理任何条目 | **中** | PID 重用会导致进程树中的节点指向错误的进程 |
| 进程树无循环保护 | **低** | 极端情况下 `is_descendant` 可能无限循环 |
| 缺少缓存指标/监控 | **低** | 没有缓存命中率、容量等可观测指标 |

✅ **已解决**：`dir_cache` 无淘汰机制、无 TTL（moka Cache + 容量 100k + TTL 1h）
...
📋 **待处理**：
2. **Exit 事件清理**：至少清理 `proc_cache` 中的已退出进程。`pid_tree` 可保留加 TTL（如 5 分钟）
3. **统一两套 PID 缓存**：只留一个，全用 DashMap（带容量控制）或全用 LruCache（加并发包装）
4. **增加 PID 重用检测**：利用 proc connector 提供的进程启动时间戳来区分新旧进程
5. **增加缓存 hit/miss 计数器**（debug 模式下）：便于性能调优
6. **`dir_cache` 改用 `LruCache`**：~~因为只有一个事件循环线程在消费，不需要并发 map~~ → ❌ 实际上多个 tokio reader task 并发读，moka 更合适（已实现）
7. **增加缓存 TTL**：→ ✅ 已实现（moka 原生支持）
