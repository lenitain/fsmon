...
- ~~两套 PID 缓存不一致~~ → proc_cache 由 proc connector 实时更新，pid_cache（LruCache 4096）仍为第二层。moka 覆盖了低频的/已退出的进程，proc_cache 未命中会回退到 pid_cache。PID 重用场景：Exec 事件覆盖更新 → 正确
...
📋 **待处理**：
4. **统一两套 PID 缓存**：proc_cache（moka）和 pid_cache（LruCache 4096）存储相同数据，仍存不一致风险。可通过在 proc_cache 上设置更小 TTL + pid_cache 作为 proc_cache 的同步镜像来解决
5. **增加 PID 重用检测**：利用 proc connector 提供的进程启动时间戳来区分新旧进程
6. **增加缓存 hit/miss 计数器**（debug 模式下）：便于性能调优
7. **进程树循环检测**：给 `is_descendant` 加 visited set 防止极端循环
8. **缓存指标导出**：moka 提供 `entry_count()` 等统计接口，可暴露给 debug 输出
