...
✅ **已解决**：
6. **PID 重用检测**：在 `ProcInfo`/`PidNode` 中存储 `start_time_ns`（来自 kernel proc_event timestamp），`get_process_info_by_pid` 缓存命中后通过对比 `/proc/{pid}/stat` starttime（jiffies→ns）来检测 PID 重用，不一致时回退到 `/proc` 读取
