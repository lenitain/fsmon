...
- ~~PID 重用风险~~ → ✅ `ProcInfo`/`PidNode` 加 `start_time_ns`（kernel timestamp），`get_process_info_by_pid` 缓存命中后验证 starttime，重用 PID 自动回退 /proc
...
⚠️ **问题**：
- ~~无上限增长~~ → ✅ 容量 65,536，W-TinyLFU 淘汰
- ~~**无循环检测**~~ → ✅ `is_descendant` / `build_chain` 均已加 visited set 检测环，防止极端情况无限循环
...
- `is_descendant` / `build_chain` 无循环保护（加 visited set 检测环）
- PID 重用风险（`ProcInfo`/`PidNode` 加 `start_time_ns`，缓存命中后验证 starttime）
- 缺少缓存指标/监控（`fsmon daemon --debug` 启动时 + 每 60s 动态打印所有 cache entry_count / 容量）
...
5. **进程树循环保护**：`is_descendant` / `build_chain` 加 visited set 检测环，防止极端情况无限循环

✅ **全部完成**：
1. `dir_cache`: DashMap → moka Cache, 容量 100k + TTL 1h
2. `proc_cache` / `pid_tree`: DashMap → moka Cache, 各 65536 + TTL 10min
3. 移除 DashMap 依赖，3 缓存统一 moka
4. 统一两套 PID 缓存，移除 pid_cache (LruCache)
5. 进程树循环保护，`is_descendant` / `build_chain` 加 visited set
6. PID 重用检测，存储 `start_time_ns` + `/proc/stat` 验证
7. 缓存指标导出，`--debug` 启动 + 每 60s 动态打印
8. **缓存参数可配置化**: CLI `--cache-dir-cap/--cache-dir-ttl/--cache-file-size/--cache-proc-ttl/--buffer-size` + `fsmon.toml [cache]` 段, 优先级 CLI > TOML > 代码默认值
